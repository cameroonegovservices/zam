...
RUN apt-get update && \
    ...
    python3.6 -m pip install --no-cache-dir --src=/src -r requirements.txt -r requirements-dev.txt && \
    apt-get remove --yes \
        build-essential \
        libpq-dev \
        python3-dev \
    ...
    rm -rf /var/lib/apt/lists/*
RUN python3.6 -m spacy download fr_core_news_md
COPY . ./
RUN python3.6 -m pip install --no-cache-dir --src=/src -e .

#
# ===== Image with testing tools =====
#

FROM webapp as tests

# Install Firefox
RUN apt-get update && apt-get -f install --yes --no-install-recommends firefox
...
#
FROM webapp as notebook
RUN python3.6 -m pip install jupyter "tornado<6"
